...
public class ArrayList {
    ...
    public boolean addLast(Object element) {
        if(size == elementData.length){
            System.out.println("addLength 실행 size =" + size);
            addLength();
        }
    ...
    public void addLength(){
        //1. 추가 length로 임시 배열을 생성
        int newLength = elementData.length+100;
        Object[] tempData = new Object[newLength];

        //2. elementData에 있는 데이터를 임시 배열로 이동
        for(int i = 0 ;i < elementData.length ; i++){
            tempData[i] = elementData[i];
        }

        //3. elementData를 추가된 length로 다시 생성
        elementData = new Object[newLength];

        //4. 데이터들을 length가 추가된 elementData로 이동
